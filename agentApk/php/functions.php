...
//AGENT AND LANDLORD SPECIFICS
else if($status=="loadContri"){
	 $uid = $_SESSION["id"];
...
}else if($status=="poLogIn"){
	$username=$_POST["username"];
	$password=$_POST["password"];
	$password=md5($password);

	$sql = mysqli_query($con,"SELECT * FROM `user` WHERE `username`='$username' AND `password`='$password' AND (`uType`='3' OR `uType`='2' OR `uType`='4')");
	if(mysqli_num_rows($sql)>0){
		while($row=mysqli_fetch_assoc($sql)){
			$_SESSION["name"] =$row["name"];
			$_SESSION["username"] = $row["username"];
