...
if(isset($_POST["registerBtn"])){
   ...
   if($password!=$rpassword){
    echo "Passwords must match";
      ...
      $sql ="INSERT INTO `user`(`id`, `username`, `name`, `email`, `password`, `contact`, `avatar`, `date_joined`,`uType`) VALUES ('','$username','$name','$email','$password','$phone','$avatar','$now','$uType')";

      $sql = mysqli_query($con,$sql);
      if($sql){

             echo "<br/><br/><p style='width:40%; text-align:center; position:absolute;left:30%;top:30%' class='alert alert-success'>Your account has been opened Successfully.<br/> 
                 Please Log in to activate,\n";
                echo "<br/><br/><a href=\"../index.php\">Log In</a></p>";
      }else{
    ...
    $password=md5($password);
    $_SESSION["signature"] = md5($password);


    $sql = mysqli_query($con,"SELECT * FROM `user` WHERE `username`='$username' AND `password`='$password' AND (`uType`='3' OR `uType`='2')");
  ...
  $p_type = $_POST["p_type"];
    $id =  $_SESSION["id"];
  $sql = mysqli_query($con,"INSERT INTO `apartment`(`id`, `name`, `region`, `district`, `town`, `hse_no`, `apo`, `description`, `digital_add`, `amount`, `contact_person`, `date_added`, `available`,`contract_type`,`p_type`,`currency`) VALUES ('','$name','$region','$district','$town','$hse_no','$apo','$description','$digital_add','$amount','$id','$now','YES','$contract_type','$p_type','$currency')");
  if($sql){
