										...
										<div>
											<table id="dynamic-table" class="table table-bordered table-hover">
												...
												<tbody>
													...
													$sql = mysqli_query($con,"SELECT any_value(`tblclients`.`userid`),any_value(`tblinvoices`.`id`) AS `invid`,any_value(`tblinvoices`.`date`) AS `date`,any_value(`tblinvoices`.`number`) AS `number`,any_value(`tblinvoices`.`prefix`) AS `prefix`,any_value(`tblinvoices`.`subtotal`) AS `subtotal`, any_value(`tblinvoices`.`adjustment`) AS `adjustment`,any_value(`tblinvoices`.`discount_total`) AS `discount_total`,any_value(`tblinvoicepaymentrecords`.`amount`) AS `amountPaid`,any_value(`tblclients`.`company`) AS `company`,any_value(`tblclients`.`phonenumber`) AS `phonenumber`,any_value(`tblinvoicepaymentrecords`.`note`) AS `note`,any_value(`tbltags`.`name`) AS `tagName`,any_value(`tblcustomers_groups`.`name`) AS `bGroup`,any_value(`tblstaff`.`firstname`),any_value(`tblstaff`.`lastname`),any_value(`tblcustomfieldsvalues`.`value`) AS `location` FROM `tblinvoices` LEFT JOIN `tblinvoicepaymentrecords` ON `tblinvoices`.`id` = `tblinvoicepaymentrecords`.`invoiceid` LEFT JOIN `tblclients` ON `tblclients`.`userid` = `tblinvoices`.`clientid` LEFT JOIN `tbltaggables` ON `tbltaggables`.`rel_id`=`tblinvoices`.`id` LEFT JOIN `tbltags` ON `tbltags`.`id`=`tbltaggables`.`tag_id` LEFT JOIN `tblcustomer_groups` ON `tblinvoices`.`clientid` = `tblcustomer_groups`.`customer_id` LEFT JOIN `tblcustomers_groups` ON `tblcustomer_groups`.`groupid` = `tblcustomers_groups`.`id` LEFT JOIN `tblstaff` ON `tblinvoices`.`sale_agent` = `tblstaff`.`staffid` LEFT JOIN `tblcustomfieldsvalues` ON `tblcustomfieldsvalues`.`relid`=`tblclients`.`userid` WHERE `tblcustomfieldsvalues`.`fieldid`='1' AND `tblinvoices`.`date` LIKE '2019%' GROUP BY `tblinvoices`.`id` ORDER BY `tblinvoices`.`id` DESC");
														if(mysqli_num_rows($sql)>0){
															$x=0;
															while ($row= mysqli_fetch_assoc($sql)) {
																# code...
